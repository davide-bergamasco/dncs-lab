...
In the case of `Subnet-A` it was also possible to use a /26 (255.255.255.192) netmask , which has 62 usable addresses as required. Instead of the /26 I preferred to choose a /25 (255.255.255.127) netmask which as more free addresses.This because one of the address must be used for the router's interface so with the /26 netmask only 61 IP addresses were free to use meanwhile with the /25 we still have 125 free IP addresses.
